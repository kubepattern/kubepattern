...
public class K8sCluster {
    ...
    public List<K8sResource> getResourcesByPlural(String plural) {
        return null;
    }
    ...
    public boolean isNeighbour(K8sResource from, K8sResource to, RelationshipType type) {
        for(K8sRelationship relationship : graph.incomingEdgesOf(from)){
            if(relationship.getType().equals(type) &&  relationship.getFromId().equals(to.getUid())) {
                return true;
            }
        }

        for(K8sRelationship relationship : graph.incomingEdgesOf(to)){
            if(relationship.getType().equals(type) &&  relationship.getFromId().equals(from.getUid())) {
                return true;
            }
        }
